...
public class DeployMojo extends GatewayAbstractMojo {
  ...
  public void doRefreshBundle() throws IOException, MojoFailureException {
    try {
      logger.info("\n\n=============Refresh Bundle================\n\n");
      logger.info("env: " + this.getProfile().getEnvironment());
      logger.info("rev: " + this.bundleRevision);
  ...
  /**
   * Clean the last bundle revision.
   */
      ...
      switch (buildOption) {
        case NULL:
          ...
          if (Options.override) {
            ...
            // deploy n-1 revision, if existing

            latestRevision = RestUtil.getLatestRevision(this.getProfile());
            if (this.latestRevision.length() > 0) {
  ...
  enum OPTIONS {
    inactive, force, validate, clean, update, override, rollback
  }
